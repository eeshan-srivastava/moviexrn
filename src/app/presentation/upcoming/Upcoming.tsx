...
const Upcoming = (props: Props) => {
    ...
    const getListOfMoviesData = async (request: MovieUpcomingRC) => {
        movieUseCase
            .getMovieUpcoming({requestContent: request})
            .then((response) => {
                const newData = response?.results.map((item: MovieUpcomingItemContent)=>{
    ...
    return (
             <PageStateComponent
                ...
                loadingComponent={
                    <DefaultLoadingView message={'Fetching upcoming movies, please wait...'} /> 
                }>
